    ...
    <div className="relative">
      ...
      {isMobile && (
        <SeletorMobile
          ...
          titulo="Selecionar transportadora"
          busca={busca}
          onBusca={setBusca}
          placeholder="Buscar por nome, CPF/CNPJ ou código…"
          onCadastrar={() => { setModal(true); setAberto(false); }}
          cadastrarLabel="Cadastrar nova transportadora"
        >
          {permitirNenhum && (
            <CardItem
              selecionado={value === ""}
              onClick={() => { onChange(""); setAberto(false); setBusca(""); }}
              titulo="Sem transporte / retirada"
            />
          )}
          {filtrados.length === 0 && !permitirNenhum ? (
            <p className="py-8 text-center text-sm text-[var(--muted)]">Nenhuma transportadora encontrada.</p>
          ) : (
            filtrados.map((t) => (
              <CardItem
                key={t.id}
                selecionado={t.id === value}
                onClick={() => { onChange(t.id); setAberto(false); setBusca(""); }}
                titulo={`${t.codigoInterno} · ${t.nome}`}
                subtitulo={t.documento || "sem documento"}
              />
            ))
          )}
        </SeletorMobile>
