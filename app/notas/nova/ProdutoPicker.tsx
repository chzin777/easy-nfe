    ...
    <div className="relative">
      ...
      {isMobile && (
        <SeletorMobile
          ...
          cadastrarLabel="Cadastrar novo produto"
        >
          {filtrados.length === 0 ? (
            <p className="py-8 text-center text-sm text-[var(--muted)]">Nenhum produto encontrado.</p>
          ) : (
            filtrados.map((p) => (
              <CardItem
                key={p.id}
                selecionado={p.id === value}
                onClick={() => { onChange(p.id); setAberto(false); setBusca(""); }}
                titulo={`${p.codigoInterno} · ${p.nome}`}
                subtitulo={`NCM ${p.ncm || "—"}`}
                direita={<span className="shrink-0 text-sm font-semibold">{formatBRL(p.preco)}</span>}
              />
            ))
          )}
        </SeletorMobile>
